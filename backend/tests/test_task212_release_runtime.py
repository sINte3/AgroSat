...
def fixture(tmp_path, health="success", restore="success"):
    ...
    prior_hash = "c" * 64
    database = "agrosat_r3_task221_runtime"
...
def test_health_recovery_identity_and_restore_path_fail_before_restore(tmp_path, case, expected):
    ...
    elif case == "database": f["identity"].write_text(json.dumps({"backup_sha256": f["backup_hash"], "rehearsal_database": "agrosat_r3_task221_other"}))
    else:
