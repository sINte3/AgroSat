...
export default function AgronomicInterpretationPanel({ fieldId }) {
  ...
  return <section className="space-y-4" aria-labelledby="agronomic-interpretation-title">
    <div className="card space-y-3 p-4"><div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <span className="rounded-full bg-agro-surface2 px-2 py-1 text-xs text-agro-text">{CONFIDENCE[data?.overall_confidence] || 'Недостаточно данных'}</span>
    </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div><p className="text-sm text-agro-muted">Состояние</p><p className="font-semibold text-agro-text">{operatorText(summary.status, SUMMARY_STATUS)}</p></div>
        <div><p className="text-sm text-agro-muted">Контекстная уверенность</p><p className="font-semibold text-agro-text">{CONFIDENCE[summary.confidence] || 'Уверенность не определена'}{Number.isFinite(summary.confidence_score) ? ` · ${summary.confidence_score}/100` : ''}</p></div>
        <div><p className="text-sm text-agro-muted">Индексы с данными</p><p className="font-semibold text-agro-text">{summary.indices_with_data ?? '—'} из 5 · с историей {summary.indices_with_sufficient_history ?? '—'}</p></div>
        <div><p className="text-sm text-agro-muted">Последнее наблюдение</p><p className="font-semibold text-agro-text">{displayDate(summary.latest_observation_date)}{Number.isFinite(summary.freshness_days) ? ` · ${summary.freshness_days} дн.` : ''}</p></div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2" aria-label="Период интерпретации">
        {RANGE_OPTIONS.map((option) => <button type="button" key={option.value} onClick={() => setRangeChoice(option.value)} className={`rounded-md px-2.5 py-1 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-agro-accent ${rangeChoice === option.value ? 'bg-agro-accent text-white' : 'bg-agro-surface2 text-agro-muted hover:text-agro-text'}`}>{option.label}</button>)}
        <button type="button" onClick={() => setRetry((value) => value + 1)} className="ml-auto rounded-md bg-agro-surface2 px-2.5 py-1 text-xs font-medium text-agro-muted hover:text-agro-text focus:outline-none focus:ring-2 focus:ring-agro-accent">Обновить</button>
      </div></div>
