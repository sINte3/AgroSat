...
const SUMMARY_ITEMS = [
  ...
  ['awaiting_satellite_verification', 'Ждут снимка'], ['improved_or_closed_recent', 'Недавно улучшено'],
];
...
const dateInputToIso = (value, endOfDay = false) => value ? new Date(`${value}${endOfDay ? 'T23:59:59' : 'T00:00:00'}`).toISOString() : '';
...
const statusTone = item => {
  if (item.is_overdue || ['critical', 'extreme', 'urgent'].includes(item.priority)) return 'border-red-200 bg-red-50 text-red-900';
  if (item.blocked || ['stale', 'external_unavailable'].includes(item.operational_status)) return 'border-amber-300 bg-amber-50 text-amber-950';
  if (item.operational_status === 'improved_closed') return 'border-emerald-200 bg-emerald-50 text-emerald-900';
  return 'border-slate-200 bg-slate-50 text-slate-800';
};

function SummaryStrip({ summary, loading }) {
  return <dl className="grid grid-cols-2 overflow-hidden rounded-xl border border-agro-border bg-white sm:grid-cols-4 xl:grid-cols-8" aria-label="Оперативная сводка">
    {SUMMARY_ITEMS.map(([key, label], index) => <div className={`min-w-0 px-3 py-3 ${index % 2 ? 'border-l' : ''} border-agro-border sm:border-l`} key={key}>
      <dt className="text-xs leading-4 text-agro-muted">{label}</dt><dd className="mt-1 text-xl font-semibold tabular-nums text-agro-text" aria-busy={loading}>{loading ? '—' : summary?.[key] ?? 0}</dd>
    </div>)}
  </dl>;
}
...
function Filters({ filters, setFilters, options, userRole, open, setOpen }) {
  ...
  return <section className="rounded-xl border border-agro-border bg-white" aria-labelledby="operational-filters-title">
    ...
    {open && <div id="operational-filter-fields" className="grid gap-3 border-t border-agro-border p-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      <FilterSelect label="Состояние" value={filters.operational_status} onChange={value => update('operational_status', value)} options={Object.entries(STATUS).map(([id, label]) => ({ id, label }))} />
...
function QueueRow({ item, selected, onSelect }) {
  ...
  return <li><button type="button" onClick={() => onSelect(item.case_key)} aria-current={selected ? 'true' : undefined} className={`block min-h-24 w-full border-b border-agro-border px-4 py-3 text-left outline-none transition-colors focus:ring-2 focus:ring-inset focus:ring-agro-accent ${selected ? 'bg-emerald-50' : 'bg-white hover:bg-agro-hover'}`}>
    <span className="flex items-start justify-between gap-3"><span className="min-w-0"><span className="block truncate font-semibold text-agro-text">{item.field_name || item.enterprise_name}</span><span className="mt-0.5 block truncate text-xs text-agro-muted">{item.crop_name || SOURCE[item.source] || item.source} · {SOURCE[item.source] || item.source}</span></span><span className={`inline-flex flex-none items-center rounded-full border px-2 py-1 text-xs font-medium ${statusTone(item)}`}>{STATUS[item.operational_status] || item.operational_status}</span></span>
...
function QueuePanel({ data, loading, selectedKey, onSelect, offset, setOffset }) {
  return <section className="min-w-0 overflow-hidden rounded-xl border border-agro-border bg-white" aria-labelledby="operational-queue-title"><div className="flex min-h-14 items-center justify-between gap-3 border-b border-agro-border px-4 py-2"><div><h2 id="operational-queue-title" className="font-semibold">Приоритетная очередь</h2><p className="text-xs text-agro-muted">{loading ? 'Обновляем…' : `Найдено: ${data.total}`}</p></div><span className="text-xs text-agro-muted">Без скрытого универсального балла</span></div><ul className="max-h-[760px] overflow-y-auto" aria-busy={loading}>
    ...
    {!loading && data.items.length === 0 && <li className="p-6 text-sm leading-6 text-agro-muted">Активных ситуаций по выбранным фильтрам нет. Сбросьте ограничения или проверьте закрытые результаты.</li>}
...
function DetailPanel({ detail, loading, error, user, online, onBack, onNavigate, onTransition, busy }) {
  ...
  return <article className="min-w-0 rounded-xl border border-agro-border bg-white px-4 py-4 sm:px-5" aria-labelledby="operational-case-title"><button type="button" className="btn-secondary mb-3 min-h-11 text-sm sm:hidden" onClick={onBack}>← К очереди</button><header className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium text-agro-muted">{SOURCE[item.source] || item.source} · {item.case_key}</p><h2 id="operational-case-title" className="mt-1 text-lg font-semibold text-agro-text">{item.title}</h2><p className="mt-1 text-sm text-agro-muted">{item.enterprise_name}{item.field_name ? ` · ${item.field_name}` : ''}{item.crop_name ? ` · ${item.crop_name}` : ''}</p></div><span className={`rounded-full border px-3 py-1.5 text-sm font-medium ${statusTone(item)}`}>{STATUS[item.operational_status] || item.operational_status}</span></header>
    ...
    <section className="border-t border-agro-border py-4" aria-labelledby="case-work-title"><h3 id="case-work-title" className="font-semibold text-agro-text">Работы и доказательства</h3>{detail.work_items.length === 0 ? <p className="mt-2 text-sm text-agro-muted">Связанных работ пока нет.</p> : <ul className="mt-2 divide-y divide-agro-border">{detail.work_items.map(work => <li key={work.id} className="py-3 first:pt-0"><div className="flex flex-wrap justify-between gap-2"><span className="font-medium">{work.instruction}</span><span className="text-sm text-agro-muted">{work.status}</span></div><p className="mt-1 text-sm text-agro-muted">{work.assignee_name || 'не назначено'} · срок {dateTime(work.due_at)}{work.result_note ? ` · ${work.result_note}` : ''}</p></li>)}</ul>}<p className="mt-2 text-sm text-agro-muted">Материалы выполнения: {detail.evidence.length}. {detail.evidence.length === 0 ? 'Файлы или метаданные не приложены.' : 'Показаны только безопасные метаданные.'}</p></section>
    ...
    <section className="border-t border-agro-border py-4" aria-labelledby="case-verification-title"><h3 id="case-verification-title" className="font-semibold text-agro-text">Проверка результата</h3>{detail.verifications.length === 0 ? <p className="mt-2 text-sm text-agro-muted">Проверка новым допустимым наблюдением ещё не создана.</p> : <ul className="mt-2 divide-y divide-agro-border">{detail.verifications.map(value => <li key={value.id} className="py-2 text-sm"><strong>{value.status}</strong><span className="ml-2 text-agro-muted">цикл {value.cycle} · {dateTime(value.created_at)}</span></li>)}</ul>}</section>
    ...
    <nav className="flex flex-wrap gap-2 border-t border-agro-border pt-4" aria-label="Связанные рабочие разделы">{item.inspection_id && <button type="button" className="btn-secondary min-h-11 text-sm focus:ring-2 focus:ring-agro-accent" onClick={() => onNavigate('field-inspection-detail', item.inspection_id)}>Осмотр #{item.inspection_id}</button>}{item.plan_id && <button type="button" className="btn-secondary min-h-11 text-sm focus:ring-2 focus:ring-agro-accent" onClick={() => onNavigate('agronomy-plans', item.plan_id)}>Меры и контроль</button>}{item.field_id && <button type="button" className="btn-secondary min-h-11 text-sm focus:ring-2 focus:ring-agro-accent" onClick={() => onNavigate('field-detail', item.field_id)}>Поле</button>}<button type="button" className="btn-secondary min-h-11 text-sm focus:ring-2 focus:ring-agro-accent" onClick={() => onNavigate('monitoring')}>Мониторинг</button></nav>
  </article>;
}
...
export default function OperationalCenterPage({ onNavigate, selectedCaseKey }) {
  ...
  const loadQueue = useCallback(async signal => { setLoadingQueue(true); setQueueError(''); try { const [nextQueue, nextSummary] = await Promise.all([listOperationalQueue(query, signal), getOperationalSummary(summaryQuery, signal)]); setQueue(nextQueue); setSummary(nextSummary); } catch (error) { if (error.name !== 'CanceledError' && error.name !== 'AbortError') setQueueError(safeError(error)); } finally { if (!signal.aborted) setLoadingQueue(false); } }, [query, summaryQuery]);
