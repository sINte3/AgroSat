"""Run one TASK_217 child process with protected values kept in memory only."""
...
def main() -> int:
    ...
    if not args.command:
        raise RuntimeError("TASK217_CHILD_COMMAND_MISSING")
    runtime_env = args.source_runtime_env.resolve(strict=True)
    values = {key: str(value) for key, value in dotenv_values(runtime_env).items() if value is not None}
    source = make_url(values.get("DATABASE_URL") or "")
    if source.database == "agrosat" or not str(source.database or "").startswith("agrosat_r3_d_pixel_ndvi_"):
    ...
    completed = subprocess.run(
        args.command, cwd=args.cwd.resolve(strict=True), env=environment,
